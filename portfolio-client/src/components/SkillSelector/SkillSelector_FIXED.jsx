...
import { Autocomplete, TextField, Chip, Box, MenuItem, Select, FormControl, InputLabel, IconButton, CircularProgress } from '@mui/material'
...
const SkillSelector = ({ title, data, editData, editMode, headers, updateEditData, keyName, showAutocomplete, showHeaders, icon }) => {
	...
	const handleAddSkill = useCallback(() => {
		...
		Object.keys(currentEditData).forEach(level => {
			if (currentEditData[level] && Array.isArray(currentEditData[level]) && currentEditData[level].some(skill => skill.name.toLowerCase() === skillName.toLowerCase())) {
				skillExists = true
			}
		})
	...
	const handleDeleteSkill = useCallback(
		(skillToDelete, level) => {
			try {
				...
				const updatedSkills = {
					...currentEditData,
					[level]: (currentEditData[level] || []).filter(skill => skill.name !== skillToDelete.name),
				}
				updateEditData(keyName, updatedSkills)
			} catch (error) {
	...
	const canAddSkill = useMemo(() => {
		return selectedSkill && selectedSkill.name && selectedSkill.name.trim() && selectedLevel && !isAdding
				...
				<Box display='flex' alignItems='center' mb={2} mt={2} gap={2} className={styles.addSkillForm}>
					{showAutocomplete ? (
						<Autocomplete
							...
							sx={{ width: 200 }}
							disabled={isAdding}
							renderInput={params => <TextField {...params} label='Select Skill' variant='outlined' size='small' />}
						/>
					) : (
						<TextField
					...
					<FormControl variant='outlined' size='small' sx={{ width: 150 }}>
						...
						<Select value={selectedLevel} onChange={event => setSelectedLevel(event.target.value)} label='Level' disabled={isAdding}>
