...
const Filter = ({ fields, filterState, onFilterChange }) => {
  ...
  const renderField = (field, index) => {
    ...
    switch (field.type) {
      case 'radio':
        return (
          <FormControl key={field.key + index} component="fieldset" sx={{ m: 1 }} style={{ width }}>
            ...
            <FormGroup>
              {field.options.map((option) => (
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      checked={(localFilterState[field.key] || []).includes(option)}
                      onChange={(e) => {
                        const newValue = (localFilterState[field.key] || []).includes(option)
                          ? (localFilterState[field.key] || []).filter((item) => item !== option)
  ...
  return (
    <Box
      ...
      id="filter"
    >
      <Grid container spacing={1} className={style.filterBar} justifyContent="space-between">
        <Grid item xs={2}>
            ...
            <Autocomplete
              ...
              onChange={(event, newValue) => {
                if (newValue && typeof newValue !== 'string') {
                  ...
                  handleChange(
                    newValue.field,
                    newValue.type === 'checkbox' ? [newValue.label] : newValue.label
                  )
                }
              }}
