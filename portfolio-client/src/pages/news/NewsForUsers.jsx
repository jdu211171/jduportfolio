...
export const NewsForUsers = () => {
		...
		try {
			...
			if (wasUnread) {
				window.dispatchEvent(
					new CustomEvent('news:unread-count-change', { detail: { delta: -1 } })
				)
			}
		} catch (err) {
	...
	const filteredSorted = useMemo(() => {
		...
		if (selectedTag) {
			list = list.filter(
				n => Array.isArray(n.hashtags) && n.hashtags.includes(selectedTag)
			)
		}
		list.sort((a, b) => {
	...
	const unreadCount = useMemo(
		() => newsData.filter(n => !n.isViewed).length,
		[newsData]
	)
	...
	const handleReadAll = async () => {
		...
		try {
			...
			if (delta > 0) {
				window.dispatchEvent(
					new CustomEvent('news:unread-count-change', {
						detail: { delta: -delta },
					})
				)
			}
	...
	useEffect(() => {
		...
		if (qp.sort && (qp.sort === 'newest' || qp.sort === 'oldest'))
			setSortBy(qp.sort)
	}, [])
			...
			<Dialog open={readAllOpen} onClose={() => setReadAllOpen(false)}>
				...
				<DialogContent>
					<div style={{ paddingTop: 8 }}>
						{t('confirmMarkAllRead') || 'Mark all news as read?'}
					</div>
				</DialogContent>
				<DialogActions>
