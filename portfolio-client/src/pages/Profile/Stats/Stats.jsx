    ...
    const fetchStudentData = async () => {
      try {
        ...
        const fetchCertificates = async () => {
          ...
          setCertificateData('main', 'JDU_JLPT', JSON.parse(studentData.jdu_japanese_certification))
          setCertificateData(
            'other',
            '日本語弁論大会学内',
            JSON.parse(studentData.japanese_speech_contest)
          )
          setCertificateData('other', 'ITコンテスト学内', JSON.parse(studentData.it_contest))
      ...
      {subTabIndex === 0 && (
        <Box my={2}>
          ...
          <CreditsProgressBar
            ...
            semester={JSON.stringify(kintoneData) !== '{}' ? kintoneData.semester?.value : 0}
          />
        </Box>
      )}
      {subTabIndex === 1 && (
        <Box my={2}>
          ...
          <CreditsProgressBar
            ...
            unit="単位"
            credits={
              JSON.stringify(kintoneData) !== '{}' ? kintoneData.partnerUniversityCredits.value : 0
            }
            semester={JSON.stringify(kintoneData) !== '{}' ? kintoneData.semester?.value : 0}
          />
        </Box>
