...
import { School, AutoStories, Face, WorkHistory, TrendingUp } from '@mui/icons-material'
...
const QA = ({
  ...
  const fetchStudent = async () => {
    try {
      if (!(Object.keys(data).length > 0)) {
        ...
        const questions = JSON.parse((await axios.get('/api/settings/studentQA')).data.value)
        let response
        if (answers) {
      ...
      if (category == 'idList') {
        ...
        for (const key in questions[category]) {
          combinedData[category][key] = {
            ...
            answer: firsttime ? '' : !answers[category][key] ? '' : answers[category][key].answer,
          }
        }
      }
    }
  ...
  return (
    <Box mb={2}>
      ...
      <>{id && ReactDOM.createPortal(portalContent, document.getElementById('saveButton'))}</>
      ...
      <Box my={2}>
        {editMode &&
          Object.entries(getCategoryData(subTabIndex)).map(([key, { question, answer }]) => (
            <QATextField
              key={key}
              data={studentQA}
              editData={editData}
