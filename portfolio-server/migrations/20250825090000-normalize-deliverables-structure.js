  ...
  async up(queryInterface, Sequelize) {
    ...
    // 1) Normalize Students.deliverables
    {
      const [students] = await sequelize.query(
        'SELECT id, "deliverables" FROM "Students" WHERE "deliverables" IS NOT NULL'
      );
      for (const row of students) {
        ...
        if (!isEqualDeliverables(list, normalized)) {
          await sequelize.query(
            'UPDATE "Students" SET "deliverables" = :deliverables WHERE id = :id',
            { replacements: { deliverables: JSON.stringify(normalized), id: row.id } }
          );
        }
      }
    }
    ...
    // 2) Normalize Drafts.profile_data.deliverables
    {
      const [drafts] = await sequelize.query(
        'SELECT id, profile_data FROM "Drafts" WHERE profile_data IS NOT NULL'
      );
      for (const row of drafts) {
        ...
        if (!isEqualDeliverables(list, normalized)) {
          ...
          await sequelize.query(
            'UPDATE "Drafts" SET profile_data = :pd WHERE id = :id',
            { replacements: { pd: JSON.stringify(newPd), id: row.id } }
          );
        }
      }
    }
  ...
  async down(queryInterface, Sequelize) {
    ...
    // Students
    {
      ...
      for (const row of students) {
        ...
        await sequelize.query(
          'UPDATE "Students" SET "deliverables" = :deliverables WHERE id = :id',
          { replacements: { deliverables: JSON.stringify(rolled), id: row.id } }
        );
      }
    }
