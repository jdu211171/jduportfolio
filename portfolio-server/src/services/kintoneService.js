...
class KintoneService {
	...
	static async syncData() {
		try {
			...
			// 1. Kintone'dan barcha ma'lumotlarni parallel ravishda olamiz (xatolar bilan ham ishlaydi)
			...
			// Muvaffaqiyatli olingan ma'lumotlarni ajratib olamiz
			const [
				students,
				staff,
				recruiters,
				...
				if (result.status === 'fulfilled') {
					console.log(
						`✅ ${appNames[index]}: ${result.value.length} yozuv olindi`
					)
					return result.value
				} else {
					console.log(
						`❌ ${appNames[index]}: Ma'lumot olishda xatolik - ${result.reason.message}`
					)
			...
			console.log(
				`📊 Jami ma'lumotlar: ${students.length} talaba, ${staff.length} xodim, ${recruiters.length} rekruter.`
			)
			...
			// Qo'shimcha ma'lumot: qaysi ilovalar ishlamadi
			...
			const message =
				failedApps.length > 0
					? `Sinxronizatsiya yakunlandi. ${
							failedApps.length
					  } ta ilovaga kirish huquqi yo'q: ${failedApps.join(', ')}`
					: "Barcha ma'lumotlar muvaffaqiyatli sinxronizatsiya qilindi."
