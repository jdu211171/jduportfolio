...
const normalizeCommaList = val => {
	...
	if (Array.isArray(val)) {
		const uniq = Array.from(
			new Set(val.map(v => String(v).trim()).filter(v => v))
		)
		return uniq.join(', ')
	}
...
exports.validateRecruiterUpdate = [
	...
	body('phone')
		...
		.optional({ nullable: true })
		.withMessage(
			'Phone number must be numeric (6-15 digits, optionally + prefix)'
		),
	body('company_name')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 100 })
		.withMessage('Company name must be a string up to 100 chars'),
	...
	body('employee_Count')
		...
		.isString()
		.isLength({ max: 500 })
		.withMessage('Employee count must be a string up to 500 chars'),
	...
	body('target_audience')
		.optional({ nullable: true })
		.customSanitizer(normalizeCommaList)
		.isString()
	...
	body('company_capital')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 500 })
		.withMessage('Capital must be a string (<=500)'),
	body('company_revenue')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 500 })
		.withMessage('Revenue must be a string (<=500)'),
	...
	body('job_description')
		...
		.isString()
		.isLength({ max: 1000 })
		.withMessage('Job description must be at most 1000 chars'),
	body('number_of_openings')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 500 })
		.withMessage('Number of openings must be a string up to 500 chars'),
	...
	body('bonus')
		...
		.isString()
		.isLength({ max: 1000 })
		.withMessage('Bonus must be a string'),
	body('allowances')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 500 })
		.withMessage('Allowances must be a string (<=500)'),
	...
	body('other_notes')
		...
		.isString()
		.isLength({ max: 500 })
		.withMessage('Other notes must be a string'),
	...
	body('japanese_level')
		...
		.isString()
		.isLength({ max: 100 })
		.withMessage('Japanese level must be a string'),
	body('application_requirements_other')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 500 })
		.withMessage('Other requirements must be a string (<=500)'),
	body('retirement_benefit')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 500 })
		.withMessage('Retirement benefit must be a string (<=500)'),
	body('telework_availability')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 200 })
		.withMessage('Telework availability must be a string'),
	body('housing_availability')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 200 })
		.withMessage('Housing availability must be a string'),
	body('relocation_support')
		.optional({ nullable: true })
		.isString()
		.withMessage('Relocation support must be a string'),
	body('airport_pickup')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 200 })
		.withMessage('Airport pickup must be a string'),
	body('intro_page_thumbnail')
		.optional({ nullable: true })
		.isString()
		.isLength({ max: 500 })
		.withMessage('Intro page thumbnail must be a string (<=500)'),
	body('intro_page_links')
		...
		.withMessage(
			'Each intro link must be an object { title?, url } with non-empty url'
		),
