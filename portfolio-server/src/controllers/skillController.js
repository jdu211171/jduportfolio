...
class SkillController {
    ...
    static async updateSkill(req, res) {
        try {
            const userType = req.user.userType.toLowerCase();
            if (userType !== 'admin' && userType !== 'staff') {
                return res.status(403).json({ error: 'Ruxsat yo\'q. Faqat Admin va Staff ko\'nikmani yangilay oladi.' });
            }
    ...
    static async deleteSkill(req, res) {
        try {
            const userType = req.user.userType.toLowerCase();
            if (userType !== 'admin' && userType !== 'staff') {
                return res.status(403).json({ error: 'Ruxsat yo\'q. Faqat Admin va Staff ko\'nikmani o\'chira oladi.' });
            }
