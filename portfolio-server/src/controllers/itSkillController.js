...
class ItSkillController {
    ...
    static async createSkill(req, res) {
        try {
            ...
            if (userType !== 'admin' && userType !== 'staff') {
                return res.status(403).json({ error: "Ruxsat yo'q. Faqat Admin va Staff ko'nikma yarata oladi." });
            }
    ...
    static async updateSkill(req, res) {
        try {
            const userType = req.user.userType.toLowerCase();
            if (userType !== 'admin' && userType !== 'staff') {
                return res.status(403).json({ error: "Ruxsat yo'q. Faqat Admin va Staff ko'nikma yangilashi mumkin." });
            }
            const { id } = req.params;
    ...
    static async deleteSkill(req, res) {
        try {
            const userType = req.user.userType.toLowerCase();
            if (userType !== 'admin' && userType !== 'staff') {
                return res.status(403).json({ error: "Ruxsat yo'q. Faqat Admin va Staff ko'nikma o'chirishi mumkin." });
            }
            const { id } = req.params;
